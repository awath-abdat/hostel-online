...
public class HostelsList extends AppCompatActivity implements LocationListener
{
  ...
  private final ArrayList<Map<String, Object>> hostels = new ArrayList<>();
  ...
  protected void onCreate(Bundle savedInstanceState)
  {
    ...
    hostelOnlineUser = receiveHostelsListIntent.getParcelableExtra("HostelOnlineUser");
    ...
    btnHostelDetails.setOnClickListener(new View.OnClickListener()
    {
      @Override
      public void onClick(View v)
      {
        Intent intentHostelDetailsSend = new Intent(HostelsList.this, HostelDetailsAndRoomList.class);
  ...
  private void fillHostelDetails()
  {
    ...
    db.collection("Hostels").get().addOnCompleteListener(task -> {
      if(task.isSuccessful())
      {
        ...
        for(QueryDocumentSnapshot doc : task.getResult())
        {
          ...
          if(hostelLocation != null && campusLocation != null && hostelLocation.get("Lat") != null && hostelLocation.get("Lng") != null)
          {
            ...
            double d = R * c;
            Log.w("Distance", String.valueOf(fromLat).concat(" " + fromLng));
            if(d > 2000) {
              continue;
            }
          }else{
          ...
          @SuppressWarnings("unchecked") Map<String, Map<String, Object>> hostelRooms = (Map<String, Map<String, Object>>)doc.getData().get("Rooms");
          @SuppressWarnings("unchecked") Map<String, Map<String, Object>> hostelLevels = (Map<String, Map<String, Object>>)(doc.getData().get("Levels"));
          ...
          while(hostelLevels != null && hostelLevels.get(levelNumber) != null && hostelRooms != null)
          {
            ...
            while(hostelRooms.get(roomLabel) != null)
            {
              if(roomType.equals("Any") && courseMate.equals("Any"))
              {
                ...
                if(hostelRooms.get(roomLabel).get("RoomType").equals("Single")) {
                  suggestedRooms.add(roomLabel);
                }
              }else if(roomType.equals("Any") && !courseMate.equals("Any")) {
                @SuppressWarnings("unchecked") ArrayList<Map<String, Object>> students = (ArrayList<Map<String, Object>>)hostelRooms.get(roomLabel).get("Students");
                for(int k = 0; k < students.size(); k++) {
                  if (courseMate.equals("Yes") && students.get(k).get("Course").equals(hostelOnlineUser.getUserCourse())) {
                    suggestedRooms.add(roomLabel);
                  } else if (courseMate.equals("No") && !students.get(k).get("Course").equals(hostelOnlineUser.getUserCourse())) {
                    suggestedRooms.add(roomLabel);
                  }
                }
              }else {
                if(!roomType.equals("Single") && hostelRooms.get(roomLabel).get("RoomType").equals(roomType)) {
                  @SuppressWarnings("unchecked") ArrayList<Map<String, Object>> students = (ArrayList<Map<String, Object>>)hostelRooms.get(roomLabel).get("Students");
                  for(int k = 0; k < students.size(); k++) {
                    if (courseMate.equals("Yes") && students.get(k).get("Course").equals(hostelOnlineUser.getUserCourse())) {
                      suggestedRooms.add(roomLabel);
          ...
          Log.w("SuggestedRooms", suggestedRooms.toString());
  ...
  private void initSource(@NonNull Style loadedMapStyle)
  {
    ...
    String USER_ICON_SOURCE_ID = "user-icon-source-id";
    GeoJsonSource userIconGeoJsonSource = new GeoJsonSource(USER_ICON_SOURCE_ID, FeatureCollection.fromFeatures(new Feature[]{Feature.fromGeometry(Point.fromLngLat(origin.longitude(), origin.latitude()))}));
  ...
  private void initLayer(@NonNull Style loadedMapStyle)
  {
    ...
    loadedMapStyle.addImage(RED_USER_ICON_ID, ResourcesCompat.getDrawable(getResources(), R.drawable.user_red, getTheme()));
